function search_fuze_zujie(request_url) {
	...
	mui.plusReady(function() {
		...
		if(user.isplayer==-1){
			mui.toast("您还未成为球员，请前往'我的'界面注册成为球员！");
		}
		// var s ="";
		...
		mui.ajax({
			...
			success: function(data) {
				...
				if ((data.data != null)) {
					var player_data = data.data;
						
					if(player_data.isCaptain==-1){
						mui.mui.toast('您当前不是球队队长，请前往创建球队!');
					}
					else{
						var search_teamallplayer_url = request_url + 'queryTeamAllPlayerById?teamId=' + player_data.isCaptain;
						mui.ajax({
							type:'GET',
